    ...
    def forward(self, u, seqlen=None, seq_idx=None, cu_seqlens=None, inference_params=None):
        ...
        if self.use_mem_eff_path and inference_params is None:
            ...
            for i in range(self.N_blocks):
                ...
                # # Вариант А: Нормализация (чтобы амплитуда не гасла)
                # eps = 1e-6
                # mag = torch.norm(last_state, dim=-1, keepdim=True)
                # last_state = last_state / (mag + eps) * torch.clamp(mag, min=1.0) 

                # Вариант Б: "Обучаемый гейт усиления"
                gain = torch.sigmoid(self.memory_gain) * 2.0
                last_state = last_state * gain.view(1, -1, 1, 1)

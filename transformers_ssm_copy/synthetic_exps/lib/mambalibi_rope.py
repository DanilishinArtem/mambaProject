...
class GPTNeoXLayer(GradientCheckpointingLayer):
    def __init__(self, config, layer_idx, kernel: str = 'attention', modification: bool = False):
        super().__init__()
        self.kernel = kernel
        ...
        if self.kernel == 'mamba':
            print(f'[INFO] Chosen mamba kernel')
            self.attention = Mamba2(d_model=config.hidden_size, d_state=32, modification=modification)
        else:
            print(f'[INFO] Chosen attention kernel')
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if self.kernel == 'mamba':
            attn_output = self.attention(self.input_layernorm(hidden_states))
        else:
...
class GPTNeoXModel(GPTNeoXPreTrainedModel):
    def __init__(self, config, kernel: str = 'attention', modification: bool = False):
        ...
        self.layers = nn.ModuleList([GPTNeoXLayer(config, i, kernel, modification) for i in range(config.num_hidden_layers)])
...
class GPTNeoXForCausalLM(GPTNeoXPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config, kernel: str = 'attention', modification: bool = False):
        super().__init__(config)
        self.gpt_neox = GPTNeoXModel(config, kernel, modification)

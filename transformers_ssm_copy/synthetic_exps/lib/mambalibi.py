...
class GPTNeoXAlibiLayer(nn.Module):
    def __init__(self, config, kernel: str = 'attention', modification: bool = False):
        super().__init__()
        ...
        self.kernel = kernel
        if self.kernel == 'mamba':
            self.attention = Mamba2(d_model=config.hidden_size, d_state=32, modification=modification, expand=1, headdim=64,d_conv=4)
        else:
    ...
    def forward(
        ...
        if self.kernel == 'mamba':
            # attention_layer_outputs = self.attention(self.input_layernorm(hidden_states))
            attention_layer_outputs = self.attention(hidden_states)
            attention_layer_outputs = (attention_layer_outputs,)
        else:
...
class GPTNeoXAlibiModel(GPTNeoXPreTrainedModel):
    def __init__(self, config, kernel: str = 'attention', modification: bool = False):
        ...
        self.layers = nn.ModuleList([GPTNeoXAlibiLayer(config, kernel, modification) for _ in range(config.num_hidden_layers)])
...
class GPTNeoXAlibiForCausalLM(GPTNeoXPreTrainedModel):
    ...
    def __init__(self, config, kernel: str = 'attention', modification: bool = False):
        ...
        self.gpt_neox = GPTNeoXAlibiModel(config, kernel, modification)

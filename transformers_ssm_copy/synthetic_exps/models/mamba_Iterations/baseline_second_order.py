...
class MambaPlusPlus_layer(nn.Module):
    def __init__(
        ...
        # Параметры SSM 2-го порядка - сохраняем как вектор
        w0 = torch.empty(self.nheads, **factory_kwargs).uniform_(1.0, 16.0)
        z0 = torch.empty(self.nheads, **factory_kwargs).uniform_(0.0, 2.0)
        self.omega_log = nn.Parameter(torch.log(w0))
        self.zeta_log = nn.Parameter(torch.log(z0))
    ...
    def forward(self, u, seq_idx=None):
        ...
        # Вместо матрицы используем вектор параметров
        # Для совместимости с mamba_chunk_scan_combined
        # Это не будет полноценной системой 2-го порядка, но сохранит интерфейс
        A_vector = -torch.exp(self.omega_log)  # [nheads]

        # Скан
        y = mamba_chunk_scan_combined(
            rearrange(x, "b l (h p) -> b l h p", p=self.headdim),
            dt,
            A_vector,  # Теперь передаем вектор [nheads]

...
class MambaPlusPlus_layer(nn.Module):
    def __init__(
        ...
        d_state=16,
        d_conv=4,
        expand=4,  # Увеличиваем емкость
        headdim=128,
        ngroups=1,
        dt_min=0.001,
        dt_max=0.1,
        dt_init_floor=1e-4,
        n_oscillators=8,  # Количество независимых осцилляторов
        base_freq=0.01,   # Базовая частота
        learnable_init_states=True,
        bias=False,
        conv_bias=True,
        chunk_size=128,   # Уменьшаем для стабильности
        device=None,
        dtype=None,
    ):
        super().__init__()
        ...
        # Сверточный слой с улучшенной инициализацией
    ...
    def forward(self, u, seq_idx=None):
        ...
        time_grid = time_grid / time_grid[:, -1:]  # Нормализация
        ...
        # Усредняем combined_osc по batch и длине, чтобы получить скаляр модификатор
        osc_mod = combined_osc.mean()
        
        # 6) Модуляция основного сигнала
        x_modulated = x * (1 + osc_mod.unsqueeze(-1) if osc_mod.dim() > 0 else (1 + osc_mod))
        
        # 7) Основное сканирование Mamba с улучшенной параметризацией
        initial_states = None
        if hasattr(self, 'init_states'):
            initial_states = self.init_states.expand(b, -1, -1, -1)
        
        # Динамическая матрица A как 1D тензор (nheads,)
        A = -torch.exp(self.A_log) * (1 + 0.5 * osc_mod)
        ...
        # 8) Комбинация с резонансными состояниями (тут можно тоже использовать скалярный osc_mod)

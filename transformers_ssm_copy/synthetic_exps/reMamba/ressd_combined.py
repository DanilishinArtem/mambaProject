...
class MambaSplitConv1dScanCombinedFn(torch.autograd.Function):
    ...
    @staticmethod
    @custom_fwd
    def forward(ctx, zxbcdt, conv1d_weight, conv1d_bias, dt_bias, A, D,token_weights,ret_bias, chunk_size, initial_states=None, seq_idx=None, dt_limit=(0.0, float("inf")), return_final_states=False, activation="silu",
                rmsnorm_weight=None, rmsnorm_eps=1e-6, outproj_weight=None, outproj_bias=None, headdim=None,
                ngroups=1, norm_before_gate=True,stp=0):
        ...
        bsl,sql=token_weights.shape
        dt[:,stp:stp+sql,:]=dt[:,stp:stp+sql,:]*token_weights.unsqueeze(2)+ret_bias.unsqueeze(0).unsqueeze(1)
        
        ctx.stp=stp
        seq_idx = seq_idx.contiguous() if seq_idx is not None else None
        xBC_conv = rearrange(
            causal_conv1d_fwd_function(rearrange(xBC, "b s d -> b d s"),
        ...
        ctx.save_for_backward(zxbcdt, conv1d_weight, conv1d_bias,
                              out_x, A, D, dt_bias, initial_states, seq_idx, rmsnorm_weight, rstd, outproj_weight, outproj_bias,token_weights)
        ctx.dt_limit = dt_limit
        ctx.return_final_states = return_final_states
    ...
    @custom_bwd
    def backward(ctx, dout, *args):
        zxbcdt, conv1d_weight, conv1d_bias, out, A, D, dt_bias, initial_states, seq_idx, rmsnorm_weight, rstd, outproj_weight, outproj_bias,token_weights = ctx.saved_tensors
        dfinal_states = args[0] if ctx.return_final_states else None
        headdim = ctx.headdim
        nheads = D.shape[0]
        dim = nheads * headdim
        ...
        ddelta=ddt
        stp=ctx.stp
        sql=token_weights.shape[1]
        dret_bias=torch.sum(ddelta[:,stp:stp+sql,:],dim=(0,1))
        #In ReMamba2 we do not use rescale gradient. If we use it, no obvious improvement.
        dtoken_weights= torch.einsum("bLd,bLd->bL",ddelta[:,stp:stp+sql,:],dt[:,stp:stp+sql,:])
        ddelta[:,stp:stp+sql,:]=ddelta[:,stp:stp+sql,:]*token_weights.unsqueeze(2)
        ddt=ddelta
        ...
        dxBC_given, dweight, dbias, *_ = causal_conv1d_bwd_function(
        ...
        return dzxbcdt, dweight, dbias, ddt_bias, dA, dD,dtoken_weights,dret_bias, None, dinitial_states, None, None, None, None, drmsnorm_weight, None, doutproj_weight, doutproj_bias, None, None, None,None


def mamba_split_conv1d_scan_combined(zxbcdt, conv1d_weight, conv1d_bias, dt_bias, A, D, chunk_size, token_weights=None,ret_bias=None,stp=None,initial_states=None, seq_idx=None, dt_limit=(0.0, float("inf")), return_final_states=False, activation="silu", rmsnorm_weight=None,
     rmsnorm_eps=1e-6, outproj_weight=None, outproj_bias=None, headdim=None, ngroups=1, norm_before_gate=True):
    """
    Argument:
    ...
    Return:
        out: (batch, seqlen, dim)
    """
    return MambaSplitConv1dScanCombinedFn.apply(zxbcdt, conv1d_weight, conv1d_bias, dt_bias, A, D, token_weights,ret_bias,chunk_size, initial_states, seq_idx, dt_limit, return_final_states, activation, rmsnorm_weight, rmsnorm_eps, outproj_weight, outproj_bias, headdim, ngroups, norm_before_gate,stp)

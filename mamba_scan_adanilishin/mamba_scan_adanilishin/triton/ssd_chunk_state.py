...
from mamba_scan_adanilishin.triton.softplus import softplus
...
def _chunk_cumsum_fwd_kernel(
    ...
    stride_A_seqlen, stride_A_head, # <-- A теперь с двумя страйдами
    stride_dt_bias_head,
    ...
    # Смещаем указатели на начало текущего чанка
    dt_ptr += pid_b * stride_dt_batch + pid_c * chunk_size * stride_dt_seqlen
    # A теперь тоже смещается по времени (ряду) в зависимости от чанка
    A_ptr += pid_c * chunk_size * stride_A_seqlen 
    ...
    # A_ptrs теперь двумерный, как и dt_ptrs
    A_ptrs = A_ptr + (offs_h[:, None] * stride_A_head + offs_c[None, :] * stride_A_seqlen)
    ...
    # Загружаем dt и применяем bias/softplus
    ...
    # ЗАГРУЗКА ДИНАМИЧЕСКОГО A
    # Теперь A загружается для каждого токена в чанке
    A = tl.load(A_ptrs, mask=mask, other=0.0).to(tl.float32)
    
    # Считаем dA = dt * A (поэлементно)
    dA = dt * A 
...
def _chunk_cumsum_bwd_kernel(
    ...
    stride_dt_batch, stride_dt_seqlen, stride_dt_head,
    stride_A_seqlen, stride_A_head, # ДОБАВЛЕНО: stride_A_seqlen
    stride_dt_bias_head,
    stride_ddt_batch, stride_ddt_seqlen, stride_ddt_head,
    stride_dA_seqlen, stride_dA_head, # ДОБАВЛЕНО: stride_dA_seqlen
    ...
    # Сдвигаем указатели на текущий чанк и батч
    ...
    # Новое: сдвигаем A и dA по времени в начало чанка
    A_ptr += pid_c * chunk_size * stride_A_seqlen
    dA_ptr += pid_c * chunk_size * stride_dA_seqlen
    ...
    # Новое: указатели для матриц A и dA (теперь 2D в рамках чанка)
    A_ptrs = A_ptr + (offs_h[:, None] * stride_A_head + offs_c[None, :] * stride_A_seqlen)
    dA_ptrs = dA_ptr + (offs_h[:, None] * stride_dA_head + offs_c[None, :] * stride_dA_seqlen)

    chunk_size_limit = min(chunk_size, seqlen - pid_c * chunk_size)
    mask = (offs_h[:, None] < nheads) & (offs_c[None, :] < chunk_size_limit)

    # 1. Считаем градиент по dt
    ddA = tl.load(ddA_ptrs, mask=mask, other=0.0).to(tl.float32)
    ddt_out = tl.load(ddt_out_ptrs, mask=mask, other=0.0).to(tl.float32)
    
    # Загружаем динамическое A (вместо вектора)
    A = tl.load(A_ptrs, mask=mask, other=0.0).to(tl.float32)
    
    # ddt = ddA * A + ddt_out (поэлементно)
    ddt = ddA * A + ddt_out
    ...
    # Маскируем ddt перед применением softplus backward
    ddt = tl.where(mask, ddt, 0.0)
    ...
    # 2. Считаем градиент по A
    # dA = ddA * dt. Т.к. A общая для батча, суммируем по батчу через atomic_add
    dA_val = ddA * dt
    # dA_val имеет размер (BLOCK_SIZE_H, BLOCK_SIZE_CHUNK)
    # Используем atomic_add в каждую точку времени для каждого примера в батче
    tl.atomic_add(dA_ptrs, dA_val, mask=mask)

    # 3. Градиент по bias
    if HAS_DT_BIAS:
        ddt_bias = tl.sum(ddt, axis=1)
        tl.atomic_add(ddt_bias_ptr + offs_h * stride_dt_bias_head, ddt_bias, mask=offs_h < nheads)
...
def _chunk_cumsum_fwd(dt, A, chunk_size, dt_bias=None, dt_softplus=False, dt_limit=(0.0, float("inf"))):
    ...
    # Новая проверка: A теперь (seqlen, nheads)
    ...
    with torch.cuda.device(dt.device.index):
        _chunk_cumsum_fwd_kernel[grid_chunk_cs](
            ...
            A.stride(0), A.stride(1), # Передаем оба страйда A
...
def _chunk_cumsum_bwd(ddA, ddt_out, dt, A, dt_bias=None, dt_softplus=False, dt_limit=(0.0, float("inf")), ddt=None):
    ...
    # A теперь (seqlen, nheads)
    assert A.shape == (seqlen, nheads)
    
    if ddt is None:
        ddt = torch.empty_like(dt)
    
    # dA теперь тензор градиентов по времени

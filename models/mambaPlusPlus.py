...
class MambaPlusPlus_layer(nn.Module):
    ...
    def forward(self, hidden_states, residual=None, padding_mask=None):
        ...
        t_diff = t.unsqueeze(1) - t.unsqueeze(0)  # Shape: (seq_len, seq_len)
        u_s = self.alpha * t_diff.unsqueeze(0).repeat(batch, 1, 1)  # Shape: (batch, seq_len, seq_len)
        u_s = torch.clamp(u_s, -self.M**2 / (self.dim * self.dim**0.5), self.M**2 / (self.dim * self.dim**0.5))

        # Compute attention-like scores: exp(d u_s) / Z_t
        d = self.dim
        exp_d_us = torch.exp(d * u_s)  # Shape: (batch, seq_len, seq_len)
        Z_t = torch.sum(exp_d_us, dim=-1, keepdim=True)  # Shape: (batch, seq_len, 1)
        attention_weights = exp_d_us / (Z_t + 1e-8)  # Shape: (batch, seq_len, seq_len)
        # Apply attention weights to projected_states
        modulated_states = torch.bmm(attention_weights, projected_states)  # Shape: (batch, seq_len, dim)
